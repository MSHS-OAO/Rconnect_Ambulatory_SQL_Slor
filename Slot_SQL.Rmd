...
raw_access_drop <- glue("DROP TABLE AMBULATORY_ACCESS_RAW_FILTERED_TEMP")
raw_access_query <- glue("CREATE TABLE AMBULATORY_ACCESS_RAW_FILTERED_TEMP AS 
SELECT d.* FROM(
    SELECT c.*, ROW_NUMBER() OVER (PARTITION BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, OVERBOOKED_YN ORDER BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, OVERBOOKED_YN, APPT_MADE_DTTM DESC) AS ROW_ID 
        FROM (
...
raw_access_index <- glue("CREATE index raw_access_index on AMBULATORY_ACCESS_RAW_FILTERED_TEMP (APPT_DATE, DEPARTMENT_ID, CAMPUS)")



### Create queries for summing scheduled and arrived hours
access_sched_arived_hours_drop <- glue("DROP TABLE AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS_TEMP")
access_sched_arived_hours_quary <- glue("CREATE TABLE AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS_TEMP AS 
...
FROM(
    (SELECT * FROM AMBULATORY_ACCESS_RAW_FILTERED_TEMP WHERE (APPT_STATUS IN ('Arrived', 'No Show', 'Scheduled')) OR 
    (APPT_STATUS IN  ('Canceled', 'Rescheduled', 'Bumped') AND TO_CHAR(APPT_DTTM, 'YYYY-MM-DD') = TO_CHAR(APPT_CANC_DTTM, 'YYYY-MM-DD'))) a
    LEFT JOIN
    (SELECT * FROM AMBULATORY_ACCESS_RAW_FILTERED_TEMP WHERE APPT_STATUS = 'Arrived') b
...
access_sched_arived_hours_index <- glue("CREATE index access_sched_arived_index on AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS_TEMP (APPT_DATE, DEPARTMENT_ID, CAMPUS)")



### Only return rows from availability that are filled slots
availability_raw_filtered_drop <- glue("DROP TABLE AMBULATORY_AVAILABILITY_RAW_FILTERED_TEMP")
availability_raw_filtered_quary <- glue("CREATE TABLE AMBULATORY_AVAILABILITY_RAW_FILTERED_TEMP AS
...
FROM(
    ...
    RIGHT JOIN
    (SELECT * FROM(
          (SELECT DISTINCT DEPARTMENT_NAME, PROV_ID, DEPT_SPECIALTY_NAME, NPI, VISIT_PROV_STAFF_RESOURCE_C FROM AMBULATORY_ACCESS_RAW_FILTERED_TEMP))) b
...
availability_raw_index <- glue("CREATE index availability_raw_index on AMBULATORY_AVAILABILITY_RAW_FILTERED_TEMP (SLOT_DATE, DEPARTMENT_ID)")


### Combine all final tables and map the campus column
slot_table_drop <- glue("DROP TABLE AMBULATORY_SLOT_TABLE_TEMP")
slot_table_quary <- glue("CREATE TABLE AMBULATORY_SLOT_TABLE_TEMP AS
                            SELECT f.*, g.holiday, 
                            ...
                            (
                              ...
                              FROM
                              (
                                SELECT a.*, b.SCHEDULED_MINS, b.ARRIVED_MINS
                                FROM
                                (
                                  SELECT * FROM AMBULATORY_AVAILABILITY_RAW_FILTERED_TEMP
                                ) a
                                LEFT JOIN
                                (
...
slot_table_index <- glue("CREATE index slot_table_index on AMBULATORY_SLOT_TABLE_TEMP (SLOT_DATE, DEPARTMENT_ID, CAMPUS)")
  ...
  tryCatch({
        ...
        dbBegin(poolcon_upt)
          ...
          dbExecute(poolcon_upt, access_sched_arived_hours_index)
             if(dbExistsTable(poolcon_upt, "AMBULATORY_AVAILABILITY_RAW_FILTERED_TEMP")){
