...
SELECT d.* FROM(
    SELECT c.*, ROW_NUMBER() OVER (PARTITION BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, MRN ORDER BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, MRN, APPT_MADE_DTTM DESC) AS ROW_ID 
        FROM (
            Select a.* , b.SLOT_DATE, b.SLOT_HOUR, b.SLOT_BEGIN_TIME FROM(
                (SELECT * FROM( 
                  SELECT PAT_ENC_CSN_ID, SITE AS CAMPUS, DEPARTMENT_NAME, 
                   DEPARTMENT_ID, PROV_NAME_WID, PROV_ID, DEPT_SPECIALTY_NAME, NPI, VISIT_PROV_STAFF_RESOURCE_C,
                    DERIVED_STATUS_DESC AS APPT_STATUS, APPT_DTTM, TO_CHAR(APPT_DTTM, 'HH24') AS APPT_HOUR, MRN,
                ...
                INNER JOIN
                (SELECT * FROM( 
                SELECT DISTINCT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SLOT_BEGIN_TIME FROM( 
                  SELECT DISTINCT DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NM_WID, PROV_ID,
                   SLOT_BEGIN_TIME, SLOT_DATE, SLOT_HOUR, TO_CHAR(SLOT_DATE, 'yyyy-mm') AS SLOT_MONTH_YEAR, SLOT_LENGTH
                    FROM V_AVAILABILITY 
...
access_sched_arived_hours_quary <- glue("CREATE TABLE AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS AS 
                           SELECT a.CAMPUS, a.DEPARTMENT_ID, a.DEPARTMENT_NAME, a.PROV_ID, a.PROV_NAME_WID, a.DEPT_SPECIALTY_NAME,                                  a.APPT_DATE, a.APPT_HOUR, a.NPI, a.VISIT_PROV_STAFF_RESOURCE_C,
                           ...
                           (SELECT * FROM AMBULATORY_ACCESS_RAW_FILTERED
                            ...
                            GROUP BY a.CAMPUS, a.DEPARTMENT_ID, a.DEPARTMENT_NAME, a.PROV_ID, a.PROV_NAME_WID, a.DEPT_SPECIALTY_NAME,                                                      a.APPT_DATE, a.APPT_HOUR, a.NPI, a.VISIT_PROV_STAFF_RESOURCE_C
                           ")
...
base_slot_query <- glue("CREATE TABLE AMBULATORY_BASE_SLOT_TABLE AS
                        ...
                        FROM
                        (SELECT DISTINCT DEPARTMENT_ID, DEPARTMENT_NAME, PROV_NM_WID, PROV_ID, SLOT_DATE, SLOT_HOUR, 
                        TO_CHAR(SLOT_DATE, 'yyyy-mm') AS SLOT_MONTH_YEAR
                        FROM V_AVAILABILITY
                        ...
                        GROUP BY DEPARTMENT_ID, DEPARTMENT_NAME, PROV_NM_WID, PROV_ID, SLOT_DATE, SLOT_HOUR, TO_CHAR(SLOT_DATE, 'yyyy-mm')) a
                        LEFT JOIN
...
availability_raw_query<- glue("CREATE TABLE AMBULATORY_AVAILABILITY_ACCESS_TABLE AS
                           ...
                           SLOT_DATE, SLOT_HOUR, TO_CHAR(SLOT_DATE, 'yyyy-mm') AS SLOT_MONTH_YEAR, SUM(SLOT_LENGTH) AS AVAILABLE_MINS
                           FROM
                           ...
                           AND SLOT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
                           AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS'))
                           GROUP BY DEPARTMENT_ID, DEPARTMENT_NAME, PROV_NM_WID, PROV_ID, SLOT_DATE, SLOT_HOUR, SLOT_MONTH_YEAR;")
...
overbook_raw_query <- glue("CREATE TABLE AMBULATORY_OVERBOOK_TABLE AS
                           SELECT c.DEPARTMENT_ID, c.PROV_ID, c.SLOT_DATE, c.SLOT_HOUR, sum(c.SLOT_LENGTH) AS OVERBOOKED_MINS_V2
                           FROM
                           ...
                           SLOT_BEGIN_TIME, SLOT_DATE, SLOT_HOUR, SLOT_LENGTH, APPT_NUMBER, UNAVAILABLE_RSN_C
                           FROM V_AVAILABILITY
                           WHERE UNAVAILABLE_RSN_C IS NULL) b
                           ON a.PAT_ENC_CSN_ID = b.PAT_ENC_CSN_ID 
                           AND a.DEPARTMENT_ID = b.DEPARTMENT_ID
                           AND a.APPT_DTTM = b.SLOT_BEGIN_TIME
                           AND a.APPT_HOUR = b.SLOT_HOUR)
                           WHERE APPT_NUMBER IN (2, 3))c
                           GROUP BY c.DEPARTMENT_ID, c.PROV_ID, c.SLOT_DATE, c.SLOT_HOUR")

overbook_raw_index <- glue("CREATE index overbook_raw_index on AMBULATORY_OVERBOOK_TABLE (SLOT_DATE, DEPARTMENT_ID)")


### updated with slot hour details
...
slot_table_quary <- glue("CREATE TABLE AMBULATORY_SLOT_TABLE AS
                            ...
                            (
                              ...
                              c.SLOT_DATE, c.SLOT_HOUR, c.SLOT_MONTH_YEAR, c.SLOT_DATE - (TO_CHAR(c.SLOT_DATE, 'D') - 1) AS APPT_WEEK, 
                              ...
                              (
                                ...
                                ON r.DEPARTMENT_ID = a.DEPARTMENT_ID AND 
                                   ...
                                   r.SLOT_MONTH_YEAR = a.SLOT_MONTH_YEAR AND
                                   r.SLOT_HOUR = a.SLOT_HOUR
                                ) s
                                LEFT JOIN
                                (
                                  SELECT * FROM AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS
                                ) b
                                ON s.DEPARTMENT_ID = b.DEPARTMENT_ID AND s.PROV_ID = b.PROV_ID AND s.SLOT_DATE = b.APPT_DATE AND s.SLOT_HOUR = b.APPT_HOUR
                              ) c
                              LEFT JOIN
                              (
                            ...
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SUM(SLOT_LENGTH) AS UNAVAILABLE_MINS
                              FROM(
                              SELECT DISTINCT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SLOT_BEGIN_TIME, SLOT_LENGTH
                              FROM V_AVAILABILITY 
                              WHERE UNAVAILABLE_RSN_C IS NOT NULL 
                              AND SLOT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS') 
                              AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS'))
                              GROUP BY DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR
                            ) h
                            ON f.DEPARTMENT_ID = h.DEPARTMENT_ID AND f.PROV_ID = h.PROV_ID AND f.SLOT_DATE = h.SLOT_DATE AND f.SLOT_HOUR = h.SLOT_HOUR
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SUM(SLOT_LENGTH) AS HELD_MINS
                              FROM(
                              SELECT DISTINCT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SLOT_BEGIN_TIME, SLOT_LENGTH
                              FROM V_AVAILABILITY 
                              WHERE UNAVAILABLE_RSN_C IS NULL 
                            ...
                            ON f.DEPARTMENT_ID = i.DEPARTMENT_ID AND f.PROV_ID = i.PROV_ID AND f.SLOT_DATE = i.SLOT_DATE AND f.SLOT_HOUR = i.SLOT_HOUR
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SUM(SLOT_LENGTH) AS PRIVATE_MINS
                              FROM(
                              SELECT DISTINCT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, SLOT_BEGIN_TIME, SLOT_LENGTH
                              FROM V_AVAILABILITY 
                              WHERE UNAVAILABLE_RSN_C IS NULL 
                            ...
                            ON f.DEPARTMENT_ID = j.DEPARTMENT_ID AND f.PROV_ID = j.PROV_ID AND f.SLOT_DATE = j.SLOT_DATE AND f.SLOT_HOUR = j.SLOT_HOUR
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR,
                            ...
                            ON f.DEPARTMENT_ID = k.DEPARTMENT_ID AND f.PROV_ID = k.PROV_ID AND f.SLOT_DATE = k.SLOT_DATE AND f.SLOT_HOUR = k.SLOT_HOUR
                            LEFT JOIN
                            (
                            SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, SLOT_HOUR, OVERBOOKED_MINS_V2
                            FROM AMBULATORY_OVERBOOK_TABLE
                            )l
                            ON f.DEPARTMENT_ID = l.DEPARTMENT_ID AND f.PROV_ID = l.PROV_ID AND f.SLOT_DATE = l.SLOT_DATE AND f.SLOT_HOUR = l.SLOT_HOUR
                            ")

slot_table_index <- glue("CREATE index slot_table_index on AMBULATORY_SLOT_TABLE (SLOT_DATE, DEPARTMENT_ID, CAMPUS)")


### Granting newly created table access to KWEONS01 Schema

grant_access <- glue("GRANT SELECT ON AMBULATORY_SLOT_TABLE to kweons01")
  ...
  tryCatch({
        ...
        dbBegin(poolcon_upt)
          ...
          dbExecute(poolcon_upt, overbook_raw_index)
           ...
           dbExecute(poolcon_upt, grant_access)
